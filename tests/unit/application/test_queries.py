...
from src.application.use_cases.get_analysis_use_case import GetAnalysisUseCase, _dict_to_field_analysis_dto
...
async def test_get_analysis_use_case_cached(mock_repos):
    uc = GetAnalysisUseCase(
        mock_repos["analysis"], mock_repos["field"], mock_repos["metrics"],
        mock_repos["obs"], mock_repos["alert"], mock_repos["cache"]
    )
    mock_repos["cache"].get_analysis.return_value = {
        ...
        "status": "COMPLETED",
        "vegetation": {
            "mean_ndvi": 0.5, "min_ndvi": 0.1, "max_ndvi": 0.9, "std_ndvi": 0.1, "trend": "STABLE", "health": "GOOD"
        },
        "alerts": []
    }
...
async def test_get_field_latest_use_case(mock_repos):
    uc = GetFieldLatestUseCase(
        mock_repos["field"], mock_repos["analysis"], mock_repos["metrics"],
        mock_repos["obs"], mock_repos["alert"], mock_repos["cache"]
    )
...
async def test_get_field_timeseries_use_case(mock_repos):
    ...
    m1 = MagicMock(analysis_id="a1", ndvi_mean=0.5, ndwi_mean=0.2, ndre_mean=0.3, ndmi_mean=0.4, savi_mean=0.5, evi2_mean=0.6)
    m2 = MagicMock(analysis_id="a2", ndvi_mean=0.6, ndwi_mean=0.1, ndre_mean=0.3, ndmi_mean=0.4, savi_mean=0.5, evi2_mean=0.6)
