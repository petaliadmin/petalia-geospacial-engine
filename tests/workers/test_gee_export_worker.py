...
def mock_export_deps():
    with (
        ...
        patch("src.infrastructure.workers.gee_export_worker.get_redis", new_callable=AsyncMock) as mock_redis,
        ...
        patch("src.infrastructure.workers.gee_export_worker._poll_and_download_export", new_callable=AsyncMock) as mock_poll,
        patch("src.infrastructure.workers.gee_export_worker._compute_rasterio_stats") as mock_stats,
        patch("src.infrastructure.workers.gee_export_worker.AlertDetectionService") as mock_alerts,
        patch("src.infrastructure.workers.gee_export_worker.SQLAnalysisRepository") as mock_analysis_repo,
        patch("src.infrastructure.workers.gee_export_worker.SQLObservationRepository") as mock_obs_repo,
        patch("src.infrastructure.workers.gee_export_worker.SQLMetricsRepository") as mock_metrics_repo,
...
async def test_gee_export_worker_success(mock_export_deps):
    ...
    mock_export_deps["sf"].return_value.return_value.__aenter__ = AsyncMock(return_value=mock_session)
    mock_export_deps["sf"].return_value.return_value.__aexit__ = AsyncMock(return_value=False)
    mock_export_deps["metrics_repo"].return_value.get_timeseries_for_field = AsyncMock(return_value=[])
    ...
    mock_fetch_result.acquisition_date = datetime(2026, 5, 18, tzinfo=timezone.utc)
